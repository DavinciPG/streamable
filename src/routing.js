...
const { getUserByEmail, createUser, getVideosForUser, getUserById, getVideoById, createNewVideo, togglePrivacy, deleteVideo} = require('./functions/functions');
...
router.put('/videos', authenticateRequest, upload.single('videoFile'), async (req, res) => {
      try {
        ...
        if (!title || !description || !videoFile) {
            return res.status(400).send('Please provide title, description, and video file.');
        }
